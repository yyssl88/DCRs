...
def main():
    ...
    # 使用新的数据路径
    data_dir = "/data_nas/DCR/split_addnoise/ml25m_test_policy"
    ...
    # 为ML25M数据集构建专门的谓词（仅针对genres列）
    if 'genres' in mcts_df.columns:
        # 统计最常见的单一类型
        all_genres = mcts_df['genres'].dropna().str.split('|').explode()
        top_genres = all_genres.value_counts().index[:15]  # 取前15个常见类型
        special_genre_preds = [f'genres contains "{genre}"' for genre in top_genres]
        # 针对空/无类型
        special_genre_preds += ['genres = ""', 'genres = "(no genres listed)"']
        # 合并到enum_predicates，避免重复
        for pred in special_genre_preds:
            if pred not in enum_predicates:
                enum_predicates.append(pred)
        print(f'✅ 为genres列添加了 {len(special_genre_preds)} 个专门谓词')
