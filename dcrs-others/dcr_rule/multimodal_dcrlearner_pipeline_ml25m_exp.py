...
def mcts_rule_discovery(df, predicates, enum_predicates, max_depth=3, n_iter=100):
    ...
    for y_pred in enum_predicates:
        ...
        for _ in range(n_iter):
            ...
            # Selection
            while node.children:
                node = node.best_child(c_param=1.4, alpha=1.0)  # 使用默认参数
            # Expansion
...
def mcts_rule_discovery_single_y_pred(args):
    ...
    for _ in range(n_iter):
        ...
        # Selection
        while node.children:
            node = node.best_child(c_param=c_param, alpha=alpha)
        # Expansion
        if not node.is_terminal(max_depth, feature_predicates):
            node.expand(feature_predicates)
            if node.children:
...
def main():
    ...
    # 补充所有rating_bin唯一值的谓词，确保不遗漏
    if 'rating_bin' in mcts_df.columns:
        rating_bin_vals = mcts_df['rating_bin'].dropna().unique()
        for val in rating_bin_vals:
            pred = f'rating_bin = "{val}"'
            if pred not in enum_predicates:
                enum_predicates.append(pred)
        print(f'✅ 为rating_bin补充所有唯一值谓词，最终枚举型谓词数: {len(enum_predicates)}')
    else:
        print('⚠️ 未找到rating_bin列，未添加枚举型谓词')
