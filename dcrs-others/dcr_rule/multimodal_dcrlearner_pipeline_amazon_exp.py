...
def main():
    ...
    #     return
    # df_train = pd.read_csv(train_csv,nrows=800)
    ...
    # df_train_csv=pd.read_csv(os.path.join(data_dir, "train_extend.csv"),nrows=800)
    # df_train_csv.to_csv(os.path.join(data_dir,"train_extend_small.csv"))
    # df_test_csv=pd.read_csv(os.path.join(data_dir, "test_extend.csv"),nrows=100)
    # df_test_csv.to_csv(os.path.join(data_dir,"test_extend_small.csv"))
    # test_dirty=pd.read_csv(os.path.join(data_dir, "test_dirty.csv"),nrows=100)
    ...
    if 'weight_unit' in mcts_df.columns:
        ...
        # 添加特定的错误模式谓词（只添加最重要的几个）
        important_errors = [
            'weight_unit = "ounce"',  # 检测ounce相关的错误
            'weight_unit = "pound"',  # 检测pound相关的错误
            'weight_unit = "kilogram"',  # 检测kilogram相关的错误
        ]
    ...
    if 'inStock' in mcts_df.columns:
        ...
        # 添加inStock的错误模式谓词
        inStock_errors = [
            'inStock = "True"',   # 检测True相关的错误
            'inStock = "False"',  # 检测False相关的错误
        ]
    ...
    if 'color' in mcts_df.columns:
        ...
        # 添加color的错误模式谓词（只添加最重要的几个）
        important_colors = [
            'color = "Black"',    # 检测黑色相关的错误
            'color = "White"',    # 检测白色相关的错误
            'color = "Brown"',    # 检测棕色相关的错误
        ]
    ...
    if 'weight_rawUnit' in mcts_df.columns:
        ...
        # 添加weight_rawUnit的错误模式谓词（只添加最重要的几个）
        important_units = [
            'weight_rawUnit = "pounds"',  # 检测pounds相关的错误
            'weight_rawUnit = "ounces"',  # 检测ounces相关的错误
            'weight_rawUnit = "grams"',   # 检测grams相关的错误
        ]
