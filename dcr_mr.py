...
def main():
    ...
    with Pool(processes=min(cpu_count(), 32), maxtasksperchild=10) as pool:    
        for noise in noise_levels:
            print(f"\n{'#'*60}\n[DCR] Processing {noise}\n{'#'*60}")
            
            # 1. Load Training Data
            mr_path = os.path.join(DCRConfig.MR_BASE_DIR, noise, "mr_metadata_train_augmented.csv")
            if not os.path.exists(mr_path): continue
            ...
            for col in DCRConfig.TARGET_COLS:
                if col in df.columns:
                    # p0_candidates.append(Predicate('binary', col, '!=', other_col=col))
                    vals = df[col].dropna().unique()
                    for v in vals:
                        # 必须加 alias='t1' 否则无法匹配 action space
                        p0_candidates.append(Predicate('unary', col, '==', val=v, alias='t1'))
            
            best_rules_global = {}

            # 3. Outer Learning Loop
            ...
            out_file = os.path.join(DCRConfig.OUT_DIR, f"dcr_rules_{noise}.json")
            ...
            # Load Test Data
            dirty_path = os.path.join(DCRConfig.MR_BASE_DIR, noise, "mr_metadata_test_dirty_augmented.csv")
            clean_path = os.path.join(DCRConfig.MR_BASE_DIR, noise, "mr_metadata_test_clean_augmented.csv")
            ...
            # Save Metrics
            metrics = {"noise": noise, "precision": p, "recall": r, "f1": f1}
            with open(os.path.join(DCRConfig.OUT_DIR, f"metrics_{noise}.json"), 'w') as f:
                json.dump(metrics, f, indent=2)

            # Save Rule Stats
            rule_stats = sorted(rule_stats, key=lambda x: x.get("fp_cnt", 0.0), reverse=True)
            stat_path = os.path.join(DCRConfig.OUT_DIR, f"rule_stats_{noise}.json")
